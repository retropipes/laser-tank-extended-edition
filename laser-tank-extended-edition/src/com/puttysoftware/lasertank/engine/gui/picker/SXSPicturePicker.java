 ...
 */
package com.puttysoftware.lasertank.engine.gui.picker;
...
public final class SXSPicturePicker {
	...
	var rows = this.choices.length / this.stackCount;
	final var extra = this.choices.length % this.stackCount;
	if (extra != 0) {
	    rows++;
	}
	this.choiceJPanel.setLayout(new GridLayout(rows, this.stackCount));
