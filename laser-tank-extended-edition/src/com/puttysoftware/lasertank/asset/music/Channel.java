...
class Channel {
	...
	private void autoVibrato() {
		var depth = this.instrument.vibratoDepth & 0x7F;
		if (depth > 0) {
			final var sweep = this.instrument.vibratoSweep & 0x7F;
			final var rate = this.instrument.vibratoRate & 0x7F;
			final var type = this.instrument.vibratoType;
	...
	private void calculateAmplitude() {
		...
		final var panRange = this.panning < 128 ? this.panning : 255 - this.panning;
		this.pann = this.panning + (panRange * (envPan - 32) >> 5);
	}

	private void calculateFrequency() {
		if (this.module.linearPeriods) {
			var per = this.period + this.vibratoAdd - (this.arpeggioAdd << 6);
			if (per < 28 || per > 7680) {
				per = 7680;
			}
			final var tone = 7680 - per;
			final var i = (tone >> 3) % 96;
			final var c = Channel.freqTable[i];
			final var m = Channel.freqTable[i + 1] - c;
			final var x = tone & 0x7;
			final var y = (m * x >> 3) + c;
			final var freq = y >> 9 - tone / 768;
			if (freq < 65536) {
			...
			var freq = this.module.c2Rate * 1712 / per;
	...
	public void resample(final int[] outBuf, final int offset, final int length, final int interpolation) {
		...
		final var lAmpl = this.ampl * (255 - this.pann) >> 8;
		final var rAmpl = this.ampl * this.pann >> 8;
		switch (interpolation) {
			...
			case LINEAR:
			default:
				this.sample.resampleLinear(this.sampleIdx, this.sampleFra, this.step, lAmpl, rAmpl, outBuf, offset,
						length);
				break;
			case SINC:
				this.sample.resampleSinc(this.sampleIdx, this.sampleFra, this.step, lAmpl, rAmpl, outBuf, offset,
						length);
				break;
		}
	}
	...
	private void trigger() {
		if (this.noteIns > 0 && this.noteIns <= this.module.numInstruments) {
			...
			final var sam = this.instrument.samples[this.instrument.keyToSample[this.noteKey < 97 ? this.noteKey : 0]];
		...
		if (this.noteKey > 0) {
			if (this.noteKey > 96) {
				...
				if (this.module.linearPeriods) {
					...
					final var tone = 768 + (key - 1 << 6) + (fineTune >> 1);
					final var i = (tone >> 3) % 96;
					final var c = Channel.periodTable[i];
					final var m = Channel.periodTable[i + 1] - c;
					final var x = tone & 0x7;
					final var y = (m * x >> 3) + c;
	...
	private void volumeSlide() {
		final var up = this.vslideParam >> 4;
		final var down = this.vslideParam & 0xF;
