...
class Sample {
	...
	public int normaliseSampleIdx(final int sampleIdx) {
		var fixedIdx = sampleIdx;
		final var loopOffset = fixedIdx - this.loopStart;
		if (loopOffset > 0) {
	...
	public void resampleLinear(final int sampleIdx, final int sampleFrac, final int step, final int leftGain,
		...
		final var loopLen = this.loopLength;
		final var loopEnd = this.loopStart + loopLen;
		var outSampleIdx = sampleIdx;
		var outSampleFrac = sampleFrac;
		outSampleIdx += Sample.DELAY;
		if (outSampleIdx >= loopEnd) {
			outSampleIdx = this.normaliseSampleIdx(outSampleIdx);
		}
		final var data = this.sampleData;
		var outIdx = offset << 1;
		final var outEnd = offset + length << 1;
		while (outIdx < outEnd) {
			if (outSampleIdx >= loopEnd) {
				if (loopLen < 2) {
					break;
			...
			final var m = data[outSampleIdx + 1] - c;
	...
	public void resampleNearest(final int sampleIdx, final int sampleFrac, final int step, final int leftGain,
		...
		final var loopLen = this.loopLength;
		final var loopEnd = this.loopStart + loopLen;
		var fixedSampleIdx = sampleIdx + Sample.DELAY;
		...
		var outIdx = offset << 1;
		final var outEnd = offset + length << 1;
		var fixedSampleFrac = sampleFrac;
		while (outIdx < outEnd) {
			if (fixedSampleIdx >= loopEnd) {
				if (loopLen < 2) {
					break;
	...
	public void resampleSinc(final int sampleIdx, final int sampleFrac, final int step, final int leftGain,
		...
		while (outIdx < outEnd) {
			...
			final var tableIdx = outSampleFrac >> Sample.TABLE_INTERP_SHIFT << Sample.LOG2_FILTER_TAPS;
			int a1 = 0, a2 = 0;
	...
	public void setSampleData(final short[] inSampleData, final int inLoopStart, final int inLoopLength,
		...
		var sampleLength = inSampleData.length;
		var outSampleData = inSampleData;
		var outLoopStart = inLoopStart;
		var outLoopLength = inLoopLength;
		...
		// Allocate new sample.
		final var newSampleLength = Sample.DELAY + sampleLength + (pingPong ? outLoopLength : 0) + Sample.FILTER_TAPS;
		final var newSampleData = new short[newSampleLength];
		...
		if (pingPong) {
			...
			final var loopEnd = outLoopStart + outLoopLength;
			for (var idx = 0; idx < outLoopLength; idx++) {
